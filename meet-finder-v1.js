// Stevnefinner v5.3 – future-only, senior/U23 only, automatic calendar feed through 2027.
(() => {
'use strict';
...
// Rankingstevner er laget for senior- og U23-utøvere.
// U20 og yngre klasser skal ikke foreslås som aktuelle stevner.
function isSeniorOrU23(m){
  const s=norm([m.name,m.competitionGroup,m.competitionSubgroup].filter(Boolean).join(' '));
  if(!s)return true;
  // U23 er eksplisitt tillatt selv om teksten også inneholder ord som "under".
  if(/\bu ?23\b|\bunder ?23\b/.test(s))return true;
  const younger=[
    /\bu ?20\b/,/\bunder ?20\b/,
    /\bu ?19\b/,/\bunder ?19\b/,
    /\bu ?18\b/,/\bunder ?18\b/,
    ...
    /\bu ?13\b/,/\bunder ?13\b/,
    /\bu ?12\b/,/\bunder ?12\b/,
    /\bjunior\b/,/\byouth\b/,/\bcadet\b/,/\bschool\b/,/\bage group\b/
  ];
  return !younger.some(re=>re.test(s));
}

function futureMatches(){const c=eventCode();return allMeets.filter(m=>isFutureThrough2027(m)&&eventMatches(m,c)&&isEurope(m)&&isSeniorOrU23(m)).sort((a,b)=>(parseDate(a.start)?.getTime()||0)-(parseDate(b.start)?.getTime()||0));}
