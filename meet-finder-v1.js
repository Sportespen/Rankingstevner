...
// individual events have no such safety net. So "0 aktuelle stevner" for e.g. 110mH could mean
// either a genuine off-season gap or that same known scrape weakness, and there's no way to
// tell those apart from outside a live deploy (no network access from here to worldathletics.org
// to check directly). This shows the same underlying counts a human would otherwise need dev
// tools to see: how many meets were fetched at all, how many are future/Europe/senior-U23
// regardless of event, and a sample of their raw discipline listings - if that sample is mostly
// meets with NO published discipline list yet, the gap is a WA data-availability issue rather
// than a bug in eventMatches() above.
function eventDiagnosticsHtml(){
  const futureEuropeAnyEvent=dedupeMeets(allMeets.filter(m=>isFutureThrough2027(m)&&isEurope(m)&&isEligibleAgeMeet(m)));
  const withPublishedDisciplines=futureEuropeAnyEvent.filter(m=>disciplineBlob(m));
  const diag={
    totalMeetsHentetFraKalenderen:allMeets.length,
    fremtidigeSeniorU23EuropaStevnerUansettOvelse:futureEuropeAnyEvent.length,
    avDisseMedPublisertProgram:withPublishedDisciplines.length,
    eksempelStevner:futureEuropeAnyEvent.slice(0,10).map(m=>({navn:m.name,dato:m.start,program:Array.isArray(m.disciplines)?m.disciplines:(m.disciplines||null)}))
  };
  return `<details style="margin-top:10px"><summary style="cursor:pointer;font-size:11px;color:#677585">Diagnostikk</summary><pre style="white-space:pre-wrap;word-break:break-word;font-size:11px;background:#f7f9fb;padding:8px;border-radius:6px;margin-top:6px">${esc(JSON.stringify(diag,null,2))}</pre></details>`;
}
