...
// TEMPORARY - re-added on request to investigate a live case (a September Zagreb meet showing
// "Innendørs") after the general diagnostics boxes were removed. Shows exactly which of the three
// venueType() branches decided the result for one card, so it doesn't need guessing from outside.
// Remove again once the investigation is done, same as the other diagnostics boxes.
function venueDiagHtml(m){
  ...
  const diag={
    ...
    rawEnvironmentField:m?.environment??null,
    explicitFieldNormalized:explicit||null,
    scannedText:s||null,
    keywordMatchIndoor:kwIndoor?kwIndoor[0]:null,
    keywordMatchOutdoor:kwOutdoor?kwOutdoor[0]:null,
    dateUsedForFallback:d?d.toISOString().slice(0,10):null,
    monthIndex:d?d.getMonth():null,
    result:venueType(m)
  };
  return `<details style="margin-top:8px;font-size:11px;color:#677585"><summary style="cursor:pointer">Diagnostikk (arena)</summary><pre style="white-space:pre-wrap;word-break:break-word;font-size:11px;background:#f7f9fb;padding:8px;border-radius:6px;margin-top:6px">${esc(JSON.stringify(diag,null,2))}</pre></details>`;
}
