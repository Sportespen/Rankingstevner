...
// Not called from render() right now - this whole section is getting new content, kept here
// so the arena/toplist wiring isn't lost in the meantime.
...
async function loadMeetDetails(id,insightHost){
  try{
    ...
    // Skip the "Ikke oppgitt" line entirely when there's nothing to show - noise on every card.
    const prizeEntries=x.prizeMoney?Object.entries(x.prizeMoney).filter(([,v])=>v):[];
    const prizesHtml=prizeEntries.length?`<div class="muted" style="margin-top:2px">Premier: ${esc(prizeEntries.map(([k,v])=>`${k}: ${v}`).join(', '))}</div>`:'';
    ...
    if(insightHost)insightHost.innerHTML=`<span>Kontakt og premier</span><strong>${contactText}</strong>${prizesHtml}${linksHtml}${infoHtml}`;
