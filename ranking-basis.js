...
(function(){
  ...
  const placingTables={
    ...
    // Was truncated to only the first 6 (distance) / 8 (tenk) places - undercounted as Placing
    // Score 0 for any real placement beyond that, instead of the real positive value further down
    // the table. wa2026-placing-audit.js (loaded later, "Final source of truth ... audited against
    // World Athletics World Ranking Rules 2026") carries the full, correct tables for these two
    // groups and patches app.js's own global placingTables with them - but that patch can only
    // reach a bare global `placingTables`, not this file's OWN separately-scoped copy of the same
    // name, so this file kept using its own stale, truncated version regardless. Replaced with the
    // exact full tables from that audited source so this file's copy is no longer out of sync.
    distance:{OW:[215,190,170,155,140,130,120,110,77,70,63,56,52,49,46,43],DF:[130,115,100,87,80,73,66,59,49,42,38,35],GW:[115,95,85,77,70,63,56,49,39,32,28,25],GL:[95,85,77,70,63,56,49,42,32,28,24,21],A:[70,63,56,49,42,35,31,27,24,21,18,15],B:[50,42,35,31,27,24,21,18,15,13,11,9],C:[35,28,24,21,18,16,14,13,11,9,8,7],D:[25,19,15,13,11,9,8,7],E:[14,11,9,8,7,6],F:[8,5,3]},
    tenk:{OW:[200,175,160,145,130,120,110,100,67,60,53,46,42,38,35,32],DF:[125,105,95,85,75,67,60,53,35,28,24,21],GW:[100,85,75,65,56,49,42,35,28,22,19,17],GL:[80,65,55,46,39,35,31,28,21,18,15,14],A:[56,49,42,35,31,27,24,21,18,15,13,11],B:[42,35,31,27,24,21,18,15,13,11,9,8],C:[32,27,22,18,15,13,12,11,10,9,8,7],D:[21,15,13,11,10,9,8,7],E:[14,10,7,6,5,4],F:[7,4,2]},
  ...
  // regular users of the site should see. Now just clears any stale box left from before this
  // change; window.__rankingstevnerOwnResults (which meet-history.js's "Historisk nivå" actually
  // depends on) is set by each caller independently of this function, so removing the rendering
