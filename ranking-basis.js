// Rankingstevner v0.19.2 – automatisk rankinggrunnlag med WA gyldighetsperioder
(function(){
  ...
  const rankingPeriodDays={standard:365,distance:365,tenk:365,combined:365};
  ...
  function validDate(r,code){const raw=r?.date;if(!raw)return false;const d=new Date(raw);if(Number.isNaN(d.getTime()))return false;const days=rankingPeriodDays[group(code)]||365;const cutoff=new Date();cutoff.setUTCDate(cutoff.getUTCDate()-days);return d>=cutoff;}
  ...
  function candidate(r,code){const g=group(code),type=g==='combined'?combinedType(r.discipline,code):(exactMatch(r.discipline,code)?'main':null);if(!type||r.legal===false||!validDate(r,code))return null;let rs=Number(r.resultScore);if(!Number.isFinite(rs)||rs<=0)rs=scoreFromTable(code,r.mark);const place=Number(r.place),cat=String(r.category||'').toUpperCase();const ps=placingTables[g]?.[cat]?.[place-1];if(!Number.isFinite(rs)||rs<=0||!Number.isFinite(place)||ps==null)return null;return {...r,type,resultScore:rs,placingScore:ps,score:rs+ps};}
  ...
  function renderBasis(basis){
    ...
    const rightHtml=basis.complete?`<div class="ranking-score-card"><div class="ranking-score-label">RANKING SCORE</div><div class="ranking-score-value">${basis.rankingScore}</div><div class="ranking-score-note">Basert på de ${basis.needed} beste tellende Performance Scores innenfor rankingperioden.</div></div>`:'';
  ...
  async function load(id){if(!id||loading)return;if(id===currentId&&allResults.length){await ensureScoring();refresh();return;}loading=true;try{const [res]=await Promise.all([fetch(`/api/wa-results?id=${encodeURIComponent(id)}&v=192`,{cache:'no-store'}),ensureScoring()]);const data=await res.json();if(data?.ok&&Array.isArray(data.results)){currentId=String(id);allResults=data.results;refresh();}}catch(_){}finally{loading=false;}}
