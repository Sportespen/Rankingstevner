// Rankingstevner v0.19.1 – automatisk rankinggrunnlag, også deløvelser fra mangekamp
(function(){
  ...
  async function ensureScoring(){
    ...
    try{const res=await fetch(SCORING_URL,{cache:'force-cache'});if(res.ok)scoringData=await res.json();}catch(_){}
    return scoringData;
  }
  function parseMark(raw,unit){
    const s=String(raw??'').trim().replace(',','.');if(!s)return NaN;
    if(unit==='seconds'&&s.includes(':')){const p=s.split(':').map(Number);if(p.some(v=>!Number.isFinite(v)))return NaN;if(p.length===2)return p[0]*60+p[1];if(p.length===3)return p[0]*3600+p[1]*60+p[2];}
    return Number(s.replace(/[^0-9.+-]/g,''));
  }
  function scoreFromTable(code,mark){
    const evt=scoringData?.[sex.value]?.[code];if(!evt)return null;
    const perf=parseMark(mark,evt.unit);if(!Number.isFinite(perf))return null;
  ...
  function candidate(r,code){
    ...
    if(!Number.isFinite(rs)||rs<=0)rs=scoreFromTable(code,r.mark);
  ...
  function basisFor(code){
    ...
    if(group(code)==='combined'){
      const mains=candidates.filter(x=>x.type==='main');const selected=[];
  ...
  async function load(id){
    ...
    if(id===currentId&&allResults.length){await ensureScoring();refresh();return;}
    loading=true;
    try{
      const [res]=await Promise.all([fetch(`/api/wa-results?id=${encodeURIComponent(id)}&v=191`,{cache:'no-store'}),ensureScoring()]);
      const data=await res.json();
      if(data?.ok&&Array.isArray(data.results)){currentId=String(id);allResults=data.results;refresh();}
...
(function(){
  function hideDuplicateScoreEditor(){const scoreInputs=document.getElementById('scoreInputs');const block=scoreInputs?.closest('.existing');if(block)block.style.display='none';}
  hideDuplicateScoreEditor();setTimeout(hideDuplicateScoreEditor,300);
})();
