...
export async function onRequestGet(context){
  ...
  try{
    // Nimarion brukes KUN for å få navn/slug. Ranking Score hentes ikke derfra.
    let name='';
    let athleteSlug='';
    try{
      const nr=await fetch(`https://worldathletics.nimarion.de/athletes/${id}`,{headers:{'User-Agent':'Mozilla/5.0 Rankingstevner/0.20.1','Accept':'application/json'}});
      if(nr.ok){
    ...
    const query=`query OfficialAthleteRanking($id: Int, $urlSlug: String!) {
      getCISSingleCompetitor(id: $id, urlSlug: $urlSlug) {
        ...
        worldRankings {
          current { rankingCalculationId eventGroup male urlSlug place rankingScore }
        }
      }
    }`;
    ...
    const gr=await fetch(WA_GRAPHQL,{
      ...
      headers:{
        ...
        'user-agent':'Mozilla/5.0 Rankingstevner/0.20.1'
      },
      body:JSON.stringify({query,operationName:'OfficialAthleteRanking',variables})
    });

    const payload=await gr.json().catch(()=>null);
    if(!gr.ok||payload?.errors?.length){
      return json({ok:false,error:'World Athletics svarte ikke med rankingdata',detail:payload?.errors?.[0]?.message||`HTTP ${gr.status}`},502);
    }
    ...
    const score=Number(hit?.rankingScore);
    ...
    return json({
      ...
      source:'World Athletics GraphQL',
      eventGroup:hit?.eventGroup||null
    });
  }catch(e){
...
function rankingEventMatches(eventGroup,code){
  ...
  const aliases={
    ...
    '100mH':['100mh'],'110mH':['110mh'],'400mH':['400mh'],'3000mSC':['3000msc','3000msteeplechase'],
