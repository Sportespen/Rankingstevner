...
(function(){
  ...
  function run(){
    ...
    // Update visible count after duplicate removal.
    ...
    if(countEl){
      ...
      countEl.childNodes.forEach(n=>{if(n.nodeType===Node.TEXT_NODE)n.remove();});
      countEl.insertBefore(document.createTextNode(String(count)),small||null);
    }
  }
  function init(){
    const host=document.getElementById('meetList');
    ...
    let timer;
    new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(run,30);}).observe(host,{childList:true,subtree:true});
    run();
  }
