// Rankingstevner v0.16.2 – sikker Tikamp/Combined Events-beregning
(() => {
  'use strict';

  // Offisiell WA 2026 Placing Score-tabell for Combined Events.
  ...
  // Kontrollpunkter fra World Athletics Result Scores for tikamp.
  // Brukes som fallback fordi den eksterne 2025-json-filen ikke inneholder Decathlon.
  ...
  function install(){
    ...
    // Korriger app.js-tabellen dersom globalen finnes.
    try{
      if(typeof placingTables!=='undefined') placingTables.combined=combinedPlacing2026;
    }catch(_){ }

    // Korriger app.js lookup slik at tikamp faktisk får Result Score.
    try{
      const original=lookupScoreFor;
      window.lookupScoreFor=function(code,raw){
        if(code==='Decathlon'){
          const n=Number(String(raw||'').replace(',','.'));
          return interpolateScore(n,decathlonAnchors);
        }
    ...
    function sync(){
      ...
      const n=Number(String(mark.value||'').replace(',','.'));
      const rs=interpolateScore(n,decathlonAnchors);
      const ps=placingScore();
      if(rs==null || !Number.isFinite(n)){
        resultScore.value=''; resultOut.textContent='–'; performanceOut.textContent='–';
      }else{
        resultScore.value=String(rs); resultOut.textContent=String(rs);
    ...
    // Trinn3 har egen lokal tabell. Overstyr visningen etter dens oppdatering.
